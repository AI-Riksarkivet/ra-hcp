...
async def list_buckets(s3: HcpStorage = Depends(get_s3_service)):
...
@router.head("/{bucket}")
async def head_bucket(bucket: str, s3: HcpStorage = Depends(get_s3_service)):
...
async def delete_bucket(bucket: str, s3: HcpStorage = Depends(get_s3_service)):
...
async def get_bucket_acl(bucket: str, s3: HcpStorage = Depends(get_s3_service)):
