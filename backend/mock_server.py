...
Start with:  uv run python mock_server.py          (port 8001)
   or:       PORT=9000 uv run python mock_server.py  (custom port)
Login with:  username=admin  password=password

All S3 operations are backed by in-memory dicts (stateful within the session).
All MAPI calls are intercepted by respx and return static fixture data.
"""
...
_mapi_logger = logging.getLogger("mock_server.mapi")
...
def _setup_mapi_routes(mock: respx.MockRouter) -> None:
    """Register respx routes returning fixture data for MAPI endpoints."""

    # ── Tenants ────────────────────────────────────────────────────
    mock.get(f"{HCP_BASE}/tenants").mock(
        side_effect=_logged_response("GET tenants", {"name": list(TENANTS.keys())}),
    )

    for tname, tdata in TENANTS.items():
        prefix = f"{HCP_BASE}/tenants/{tname}"
        mock.get(prefix).mock(
            side_effect=_logged_response(f"GET tenant/{tname}", tdata),
        )
...
def _seed_s3(s3: MockS3Service) -> None:
...
if __name__ == "__main__":
    ...
    logger.info("Starting HCP mock development server...")
    logger.info("Credentials: username=admin  password=password")
