...
async def test_get_node_statistics(client: AsyncClient, auth_headers: dict, hcp_mock: respx.Router):
...
async def test_get_node_statistics_hcp_error(client: AsyncClient, auth_headers: dict, hcp_mock: respx.Router):
    hcp_mock.get(f"{HCP_BASE}/nodes/statistics").mock(
...
async def test_get_service_statistics(client: AsyncClient, auth_headers: dict, hcp_mock: respx.Router):
...
async def test_get_service_statistics_hcp_error(client: AsyncClient, auth_headers: dict, hcp_mock: respx.Router):
    hcp_mock.get(f"{HCP_BASE}/services/statistics").mock(
        return_value=httpx.Response(403)
    )
    resp = await client.get("/api/v1/mapi/services/statistics", headers=auth_headers)
