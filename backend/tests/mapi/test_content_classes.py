...
async def test_list_content_classes(client: AsyncClient, auth_headers: dict, hcp_mock: respx.Router):
...
async def test_create_content_class(client: AsyncClient, auth_headers: dict, hcp_mock: respx.Router):
...
async def test_get_content_class(client: AsyncClient, auth_headers: dict, hcp_mock: respx.Router):
...
async def test_get_content_class_not_found(client: AsyncClient, auth_headers: dict, hcp_mock: respx.Router):
...
async def test_check_content_class(client: AsyncClient, auth_headers: dict, hcp_mock: respx.Router):
    hcp_mock.head(f"{MAPI_PREFIX}/{CC}").mock(
        return_value=httpx.Response(200)
    )
...
async def test_update_content_class(client: AsyncClient, auth_headers: dict, hcp_mock: respx.Router):
    route = hcp_mock.post(f"{MAPI_PREFIX}/{CC}").mock(
        return_value=httpx.Response(200)
    )
    resp = await client.post(
        f"{PREFIX}/{CC}", headers=auth_headers, json={"contentProperties": []},
    )
    assert resp.status_code == 200
    assert route.called


async def test_delete_content_class(client: AsyncClient, auth_headers: dict, hcp_mock: respx.Router):
