...
async def test_get_log_status(client: AsyncClient, auth_headers: dict, hcp_mock: respx.Router):
...
async def test_log_mark(client: AsyncClient, auth_headers: dict, hcp_mock: respx.Router):
    route = hcp_mock.post(f"{MAPI_PREFIX}").mock(
...
async def test_log_cancel(client: AsyncClient, auth_headers: dict, hcp_mock: respx.Router):
    route = hcp_mock.post(f"{MAPI_PREFIX}").mock(
        return_value=httpx.Response(200)
    )
...
async def test_prepare_logs(client: AsyncClient, auth_headers: dict, hcp_mock: respx.Router):
    route = hcp_mock.post(f"{MAPI_PREFIX}/prepare").mock(
        return_value=httpx.Response(200)
    )
    resp = await client.post(
        f"{PREFIX}/prepare", headers=auth_headers, json={"startDate": "2024-01-01"},
    )
    assert resp.status_code == 200
    assert route.called


async def test_download_logs(client: AsyncClient, auth_headers: dict, hcp_mock: respx.Router):
...
async def test_logs_hcp_error(client: AsyncClient, auth_headers: dict, hcp_mock: respx.Router):
    hcp_mock.get(f"{MAPI_PREFIX}").mock(
