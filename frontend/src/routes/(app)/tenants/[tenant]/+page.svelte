...
<svelte:head>
	{#await tenant then t}
		<title>{t.name} - HCP Admin Console</title>
	{/await}
</svelte:head>
...
<div class="space-y-6">
	...
	<div>
		...
		{:then ns}
			<DataTable columns={nsColumns} data={ns} emptyMessage="No namespaces found for this tenant" />
		{/await}
	</div>
</div>
