...
export const get_tenant = query(z.object({ tenant: z.string() }), async ({ tenant }) => {
	try {
...
export const get_tenant_statistics = query(z.object({ tenant: z.string() }), async ({ tenant }) => {
	try {
...
export const get_tenant_chargeback = query(z.object({ tenant: z.string() }), async ({ tenant }) => {
	try {
		const res = await apiFetch(`/api/v1/mapi/tenants/${tenant}/chargebackReport`);
...
export const get_tenant_settings = query(z.object({ tenant: z.string() }), async ({ tenant }) => {
	const base = `/api/v1/mapi/tenants/${tenant}`;
	const keys = ['consoleSecurity', 'contactInfo', 'permissions', 'namespaceDefaults'] as const;
...
export const update_namespace_defaults = command(
	z.object({ tenant: z.string(), body: z.record(z.string(), z.unknown()) }),
	async ({ tenant, body }) => {
		const res = await apiFetch(`/api/v1/mapi/tenants/${tenant}/namespaceDefaults`, {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify(body),
		});
