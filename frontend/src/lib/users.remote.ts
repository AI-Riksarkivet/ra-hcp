...
export const get_users = query(z.object({ tenant: z.string() }), async ({ tenant }) => {
	try {
	...
	async ({ tenant, username }) => {
		try {
			const res = await apiFetch(
				`/api/v1/mapi/tenants/${tenant}/userAccounts/${encodeURIComponent(
					username
...
export const get_user = query(
	...
	async ({ tenant, username }) => {
		try {
			const res = await apiFetch(
				`/api/v1/mapi/tenants/${tenant}/userAccounts/${encodeURIComponent(username)}`
			);
...
export const change_password = command(
	...
	async ({ tenant, username, password }) => {
		const res = await apiFetch(
			`/api/v1/mapi/tenants/${tenant}/userAccounts/${encodeURIComponent(username)}/changePassword`,
			{
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
...
export const get_groups = query(z.object({ tenant: z.string() }), async ({ tenant }) => {
	try {
		const res = await apiFetch(`/api/v1/mapi/tenants/${tenant}/groupAccounts`);
		if (res.ok) {
			const data = await res.json();
