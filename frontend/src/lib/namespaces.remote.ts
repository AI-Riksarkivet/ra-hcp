...
export const get_namespaces = query(z.object({ tenant: z.string() }), async ({ tenant }) => {
	try {
				...
				names.map(async (n) => {
					try {
						const r = await apiFetch(
							`/api/v1/mapi/tenants/${tenant}/namespaces/${encodeURIComponent(n)}`
						);
...
export const get_ns_protocols = query(
	...
	async ({ tenant, name }) => {
		try {
			const res = await apiFetch(
				`/api/v1/mapi/tenants/${tenant}/namespaces/${encodeURIComponent(name)}/protocols`
			);
...
export const update_ns_protocol = command(
	...
	async ({ tenant, name, protocol, body }) => {
		const res = await apiFetch(
			`/api/v1/mapi/tenants/${tenant}/namespaces/${encodeURIComponent(name)}/protocols/${protocol}`,
			{
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
...
export const get_ns_permissions = query(
	...
	async ({ tenant, name }) => {
		try {
			const res = await apiFetch(
				`/api/v1/mapi/tenants/${tenant}/namespaces/${encodeURIComponent(name)}/permissions`
			);
